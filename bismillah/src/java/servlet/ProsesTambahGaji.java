 ...
 * @author ACER
 */
@WebServlet(name = "ProsesTambahGaji", urlPatterns = {"/ProsesTambahGaji"})
public class ProsesTambahGaji extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String namapenerima = request.getParameter("namapenerima");
        ...
        if (idgaji.isEmpty() || namapenerima.isEmpty() || jumlahgaji1.isEmpty()) {//validasi isian masukan (kosong/tidak)
            ...
            RequestDispatcher rdp = request.getRequestDispatcher("pages/tambahgaji.jsp");
            rdp.forward(request, response);
        
        } else if (!jumlahgaji1.matches("[0-9]*")) { //validasi input jumlah gaji harus angka
            request.setAttribute("error", "Maaf, data gaji gagal disimpan. Anggota keluarga harus berupa angka.");
            RequestDispatcher rdp = request.getRequestDispatcher("pages/tambahgaji.jsp");
            rdp.forward(request, response);
